...
watch(() => props.prestation, (newVal) => {
    editedPrestation.value = {
        prestation_name: '',
        prestation_code: '',
        specialty_id: null,
        ...newVal
    };
    // Reset errors when prestation changes
    errors.value = {
        prestation_name: false,
        prestation_code: false,
        specialty_id: false
...
const validate = () => {
    ...
    if (!editedPrestation.value.prestation_name?.trim()) {
        errors.value.prestation_name = true;
        isValid = false;
    }

    if (!editedPrestation.value.prestation_code?.trim()) {
        errors.value.prestation_code = true;
        isValid = false;
    }

    if (!editedPrestation.value.specialty_id) {
        ...
        <div class="flex flex-col gap-3">
            <div class="field">
                ...
                <InputText size="small" id="prestation_code" v-model.trim="editedPrestation.prestation_code"
                    class="w-full" placeholder="Prestation Code" :class="{ 'p-invalid': errors.prestation_code }" />
            ...
            <div class="field">
                ...
                <InputText size="small" id="prestation_name" v-model.trim="editedPrestation.prestation_name"
                    class="w-full" placeholder="Prestation Name" :class="{ 'p-invalid': errors.prestation_name }" />
