  ...
  <div class="w-full p-4">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-center mb-4 gap-2">
      ...
      <Button v-if="props.contractState === 'Pending'" label="Add Prestation" icon="pi pi-plus"
        @click="openAddDialog" />
      <Button v-if="avenantpage === 'yes' && avenantState === 'Pending'" label="Add Prestation" icon="pi pi-plus"
        @click="openAddDialog" />
    </div>
    ...
    <DataTable :value="filteredItems" stripedRows paginator :rows="8" tableStyle="min-width: 50rem">
      ...
      <Column v-if="props.contractState === 'Pending'" header="Actions">
        <template #body="slotProps">
          <Button icon="pi pi-pencil" severity="secondary" size="small" variant="outlined" 
            @click="openEditDialog(slotProps.data)" />
        </template>
      </Column>
      <Column v-if=" avenantpage === 'yes' && avenantState === 'Pending' && (props.contractState==='Pending'||props.contractState==='Active')" header="Avenant">
        <template #body="slotProps">
...
const fetchPrestations = async () => {
  try {
    const response = await axios.get(`${API_BASE_URL}/api/convention/prestationprices/avenant/${props.avenantid}`);
...
const fetchPrestationList = async () => {
  try {
    const response = await axios.get(`${API_BASE_URL}/api/convention/prestationlists/avenant/${props.avenantid}`);
...
const saveService = async () => {
  ...
  try {
    // Fixed: Make sure all required fields are included and values are properly formatted
    const serviceData = {
      prestation_list_id: newService.value.prestation_list_id,
      price: newService.value.price,
      patient_part: newService.value.patientPart,
      tva: newService.value.tva,  // Include TVA in the service data
      updated_by_id: null, // Assuming this is meant to be null for new entries
      avenant_id: props.avenantid, // Redundant but included for clarity
      head: 'no',
      // No annex_id as it should be handled by the backend based on specialty
    };

    // API path: This assumes the API knows the avenantId from the URL param
    await axios.post(`${API_BASE_URL}/api/convention/prestationprices/add-prestation-avenant/${props.avenantid}`, serviceData);
    ...
    let errorMsg = 'Failed to add prestation';
...
const updateService = async () => {
  try {
    const updateData = {
      ...
      tva: selectedService.value.tva,  // Include TVA in the update data
      updated_by_id: selectedService.value.updated_by_id,
      prestation_list_id: selectedService.value.prestation_list_id,
      avenant_id: selectedService.value.avenant_id,
      head: 'no'
    };
...
const deleteItem = async () => {
  ...
  try {
    await axios.delete(`${API_BASE_URL}/api/convention/prestationprices/delete-avenant-prestation/${itemToDelete.value.id}`);
