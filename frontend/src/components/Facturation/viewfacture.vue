<template>
    <div class="card">
      <DataTable v-if="products.length > 0" :value="products" tableStyle="min-width: 50rem">
        ...
        <Column field="borderauNumber" header="N° Borderau">
      ...
      <Dialog v-model:visible="showFactureModal" modal header="Détails de la Facture" :style="{ width: '50vw' }">
        ...
        <template #footer>
          <Button v-if="selectedFacture" label="Fermer" icon="pi pi-check" class="p-button-sm "
  ...
  const getSeverity = (status) => {
    switch (status) {
      case 'facturée': return 'success';
      case 'validée': return 'warn';
