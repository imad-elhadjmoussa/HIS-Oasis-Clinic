const companyModel = require('../../models/convention_models/company.model');
...
const createCompany = async (req, res) => {
    try {
        ...
        // 4. Check if this is the first company in the database
        const isFirst = await companyModel.isFirstCompany();
        let newCompany;
        let isPublic = false;
        
        if (isFirst) {
            // Create as public company with general contract
            newCompany = await companyModel.createPublicCompanyWithGeneralContract(req.body);
            isPublic = true;
        } else {
            // Create as normal company
            newCompany = await companyModel.createCompany(req.body);
        }
        
        res.status(201).json({ 
            message: isPublic ? 
                "Public company with general contract created successfully" : 
